...
impl Editor {
    ...
    fn move_cursor(&mut self, key: Key) {
        let Position { mut y, mut x } = self.current_position;
        ...
        match key {
            Key::Up | Key::Char('j') => y = y.saturating_sub(1),
            Key::Down | Key::Char('k') => {
                if y < height {
    ...
    fn darw_welcome_message(&self) {
        let mut welcome_message = format!("More editor -- version {}", VERSION);
        let width = self.terminal.size().width as usize;
